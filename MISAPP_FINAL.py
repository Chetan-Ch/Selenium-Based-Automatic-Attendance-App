...
def open_file():
    ...
    global isclicked
    global f1
    f = filedialog.askopenfile(initialdir="E:/2020-21/Control System/CS_Attendence", title="Select A File", filetypes=(("csv files", "*.csv"),("all files", "*.*")))
    fil = f
    f1=f
...
btn = Button(tab1, text ='Select csv file', command =lambda :open_file())
Upload_Attendance= Button(tab1, text ='Upload Attendance in MIS', command =lambda :Validation_Final())
btn.pack()
btn.place(relx=0.45,rely=0.2,anchor='nw')
...
def Update_Final():
    ...
    messagebox.showinfo("showinfo", "Updation Completed")
...
def automation():
    ...
    try:
        ...
        try:
            ...
            messagebox.showerror("showerror", "{}".format(e.msg))
            print("line 650")
            driver.quit()
        ...
        driver.find_element_by_link_text("Student Attendance").click()

        driver.find_element_by_link_text("Mark Student Attendance").click()

        x=btn1.get()
        if (x==1):
            try:
                driver.find_element_by_xpath("//input[@id='{}']".format(x)).click()
                ...
                driver.find_element_by_xpath("//input[@value='Show']").click()
                time.sleep(5)
                try:
            ...
            driver.find_element_by_xpath("//input[@value='MarkPresent']").click()
            ...
            try:
                if driver.find_element_by_xpath("//*[@id='msg_display']"):
                    messagebox.showerror("showerror", "Attendance Marked Successfully")
                    driver.close()
